...
```
Elements live on `slide.elements?: Element[]` and render in `ElementsLayer` over the layout template. The editor tracks a **selection set** of element ids (`Editor.selElIds`); geometry helpers are in `lib/elements.js`. See §9. (Multi-select, rotate, rotate-aware resize, opacity, and shift-add-to-marquee are all wired.)
...
| Tools | Select / Pen / Image, 10 Shapes | 🟢 a **shape tool draws** on the canvas — drag to size (or click for a default), creating an on-slide `slide.elements` shape (grid-snapped + bounds-clamped via `snapDrawnBox`/`clampElement`), then reverting to Select; **Text Box** & **Image** insert an element (Image via the file picker), and all element-creating actions exit draw mode. 🟢 **Pen** draws a freehand stroke → a `path` element |
...
| Misc | auto-arrange (✨), timeline toggle, version history | 🟢 auto-arrange packs a 2+ selection into a centred grid (`autoArrangeElements`); 🟡 timeline toggles drawer; version history 🔴 |
...
- 🟢 **Inline text editing** — double-click any HTML text field on the canvas (titles, eyebrows, bodies, subtitles, agenda/list/kpi/split/risks items, table cells) to edit it in place. Each field is an `<EditableText>` that's read-only off-canvas and an uncontrolled `contentEditable` on it; a commit (blur or Enter) builds a `fieldPatch` (`lib/slideEdit.js`) — rebuilding the whole array for nested edits — and routes through `onApplyAIPatch` → `sanitizeSlidePatch`/`applySlidePatch`, the same validated path the Co-pilot uses. The interaction overlay hit-tests through to the text on double-click. 🟢 **All roadmap labels** — lane names, month-axis labels, and milestone labels — are inline-editable too: on the canvas each renders via `EdLabel` as an HTML `<EditableText>` inside an SVG `<foreignObject>` (read-only surfaces keep the plain SVG `<text>`, so thumbnails/sorter/presenter stay pixel-identical; the edit box is capped + centred per lane so dense roadmaps don't misplace it). Each commits the **full materialized** `{ months, lanes, todayIndex }` model (`renameLane` / `renameMonth` / `relabelMilestone` in `lib/roadmapSpec.js`) through the same gate the Data-tab editor uses — committing the whole model (not a sparse field) is required so editing one label on a demo roadmap can't flip `usingDefaultLanes` and drop the default lanes / months / TODAY marker. 🟢 **Every chart label** is inline-editable the same way: category (x-axis) labels on line / bar / area (`EdLabel` with `align='middle'`), the **donut slice legend** labels, and the **multi-series legend** names — each commits the full `{ chart: { categories, series } }` via `renameCategory` / `renameSeries` in `lib/chartSpec.js` (so a data-less chart keeps its default series/categories). With the roadmap labels above, **all SVG-rendered chart/roadmap text is now editable in place** — nothing is Data-tab-only.
- 🟢 **Inline text formatting** — while editing a fixed template field, a floating **FormatToolbar** (`components/editor/FormatToolbar.jsx`) appears above it with **bold / italic / underline**, a **size** stepper, and a **colour** picker. Formatting is stored per-field on `slide.fmt[fmtKey] = { bold, italic, underline, fontSize, color }` (`lib/slideFmt.js`), merged over the field's template style by the renderer's `E()` helper on every surface (canvas + thumbnails/sorter/presenter) and written through the same validated `fieldPatch` → `applyAIPatch` gate (`fmt` is shape-checked in `deckUtils.js`). The toolbar tracks `document` focus on `[data-fmt-key]` fields and keeps the field focused (mousedown `preventDefault`); un-setting a toggle returns the field to its template baseline (only set props are emitted). 🟢 Covers **fixed top-level fields** (title/subtitle/eyebrow/…) **and per-item fields** — agenda/list/kpi/stat items and table headers/cells — via index-keyed paths (`items.0.t`, `kpis.1.val`, `rows.0.1`, `columns.2`). The formattable vocabulary (`isFormattablePath`/`isFormattableKey` in `lib/slideFmt.js`) is the single source the renderer's `E()` and the patch gate (`isFmtMap`) both consult, so the gate accepts exactly what renders. Per-item fmt is index-keyed: stable through inline edits; reordering or deleting agenda/list items in the **Data** tab remaps the index keys (`remapCollectionFmt`) so formatting follows its item, and an AI wholesale item rewrite re-applies it by position (a rewrite that shrinks a collection leaves orphan keys that render nothing). 🟢 **PPTX export reads `fmt`** — `pptxExport.js` merges each field's `fmt` (`fmtOpts`/`fmtText`) over the layout builder's base text options at every emit site (top-level + per-item, including table cells), so bold/italic/underline/colour survive export. 🟡 `fmt.fontSize` export is wired **per-layout** via a canvas↔export baseline ratio: `fmt.fontSize` is an absolute canvas-px size and the builders use an independently hand-tuned pt scale (not `PT(px)`), so `fmtText` scales each builder's pt baseline by `fmt.fontSize ÷ basePx` for fields whose builder passes a `basePx` (the field's canvas default px). The baseline is `CANVAS_BASELINE_PX` (`lib/fontBaselines.js`), **single-sourced with the renderer** so the ratio's denominator can't drift from what the canvas actually renders. 🟢 opted in so far: **every layout title** (text/cover/divider/agenda/list/kpi/chart/split/table/risks/roadmap/thanks), **text** body, the **agenda**/**list** per-item fields, the **kpi/split/risks per-item data fields** (kpi values/labels/deltas, split stats, risk item title+desc — index-keyed, `basePx` keyed by field-type since every row shares the canvas px), and the top-level **split body** + **thanks subtitle**. A field resized on the canvas exports at the same proportion. ⚪ what remains is the **table** cells/headers (a separate `addTable`/`fmtFor` path) and the **cover subtitle** (deferred — its canvas size is inherited from the cover layout, not authored per-field). (Some formattable fields are **canvas-only** — kpi `target`, the chart/roadmap eyebrows, the chart subtitle — and never reach the export, so they sit outside export parity; that leaves chart and roadmap with only their title as `fmt`-sized export text, which completes them.)
- 🟢 Direct manipulation of `slide.elements` (§9): click to select, **shift-click to multi-select** (additive toggle), **marquee** (drag empty canvas to rubber-band-select overlapping elements; click empty space to deselect), drag to move (a drag on any member moves the whole selection), 8-handle resize and a **rotate handle** when exactly one element is selected. A drag commits one atomic deck update on pointer-up (no per-frame PUT). **Delete/Backspace** removes the selection, **⌘/Ctrl-D** duplicates it (offset clones with fresh ids, then selected), **⌘C / ⌘X / ⌘V** copy / cut / paste elements (an in-app clipboard — paste lands offset clones on the *current* slide, so it works across slides and repeats with a cascade), and **arrow keys** nudge the selection one grid step (Shift = ×5) — all suppressed while typing in a field (so ⌘C/⌘V there stay native). **align (left/center/right + top/middle/bottom)** acts on a 2+ selection, **distribute** evens the gaps of a 3+ selection, **auto-arrange** packs a 2+ selection into a near-square grid of uniform cells centred on the selection (`autoArrangeElements` — lays out both axes at once, unlike align/distribute), and **bring to front / send to back** z-orders the single selected element (`reorderElement` reorders `slide.elements`, whose array order is the paint order on both canvas and export) — the Arrange buttons are disabled until the right number of elements is selected. 🟢 **Group / ungroup** — **⌘/Ctrl-G** stamps a shared `groupId` on a 2+ selection and **⌘/Ctrl-Shift-G** strips it (`groupElements` / `ungroupElements` in `lib/elements.js`, gate-validated `groupId` in `deckUtils`); selecting any member — by click or marquee — pulls in the whole group (`expandToGroups`), so it moves / deletes / duplicates as a unit, reusing the existing multi-select transforms. Grouping is a canvas-editing relationship only: the export flattens it (each element keeps its absolute geometry), so no export/parity change. 🟢 **A 2+ selection (group or not) gets a group transform frame** — an axis-aligned bbox whose 8 handles scale every member proportionally from the edge opposite the drag (`resizeGroup`, bbox floored at the min) and whose knob rotates the whole selection about its centre (`rotateGroup` — each child orbits the centre and its own `rot` advances); during a rotate drag the frame itself tilts rigidly with that angle (a `frameTilt` preview around the pre-drag bbox) rather than re-fitting an axis-aligned box to the rotated children each frame. Both bake into each child's geometry (no group entity), so the export still flattens with no group concept. The single-element frame (with rotated-box handles) still applies for exactly one selected. 🟢 **Shift-add-to-marquee** — holding Shift while sweeping the marquee adds the swept elements to the current selection (union) rather than replacing it, and a Shift-click on empty space keeps the selection instead of deselecting; a plain sweep still replaces and a bare click still deselects.
- 🟢 **Drawing shapes** — with a shape tool active (`ShapeMenu`), a drag on the canvas draws that shape at the swept box and a click drops a default-size one at the point; the gesture (`startDraw` in `CanvasSlide`) reuses the marquee preview, grid-snaps + per-type-floors the box (`snapDrawnBox`), commits through `onAddElement` → `createElement` (which `clampElement`s the new element on-slide), selects it, and reverts to Select. In draw mode the overlay shows a crosshair, element hit boxes are click-through, and the selection frame hides. 🟢 **Freehand Pen** — a sweep (`startPen`) captures slide-coord points (decimated to a `PEN_MIN_GAP` so a slow stroke stays lean), previews a live polyline, and commits a `path` element on release (`pathFromStroke` → box + 0..1 box-relative points, so it rides every move/resize/rotate untouched); a click / sub-threshold tremor draws nothing. The `path` renders as an SVG polyline (`vector-effect:non-scaling-stroke`) and exports faithfully as pptxgen custom geometry (`custGeom` — points scaled to box inches, no fill → `<a:noFill/>`). Its points are gate-validated (≥2 finite pairs) and the renderer/export drop malformed points via the shared `isFinitePoint`.
- 🟢 Right-click context menu — **Paste** (`onPasteElements`), **Generate with AI** (opens the Co-pilot drawer), **Change layout** / **Apply theme** (drill-in choosers built from the toolbar's option lists), **Duplicate** / **Delete** slide — all wired (`SlideEditor.jsx`). Positions from the canvas rect and dismisses on any outside click.

#### 7.2.5 Right inspector (`InspectorPane` / `FloatingInspector`) 🟡
Tabs: **Design / Properties / Data / Animate**.
- **Design panel** 🟡 — 🟢 **Theme swatches** apply a deck theme (`onChangeTheme`) and mark the active one; 🟢 the **accent token** mirrors the live deck theme (its swatch + value track `deck.theme`, single-sourced with the swatches via `accentColor` in `ThemeMenu.jsx`); 🟢 **Components rail** inserts a component slide (`onAddComponent` → `createComponentSlide`). 🟢 the **Layout grid** swaps the current slide's layout (`onChangeLayout`, sharing `LAYOUT_OPTIONS` + the `.layout-grid`/`.layout-opt` styling with the toolbar Layout menu so the two pickers can't drift; the active card mirrors the slide's layout). 🟢 the **H1 token** is a real **deck-level heading scale** — a presets select (`HEADING_SCALES`) that sets `deck.headingScale`, multiplying every layout's title baseline on the canvas (`headingPx`, single-sourced into `<Slide>`'s `<h1>` size) *and* the PPTX export (the title's pt + basePx both scale via `fmtText`'s `scale`, so an unformatted title scales while an absolute per-title `fmt.fontSize` cancels out — canvas↔export parity); the row shows the live resulting current-slide title px. `resolveHeadingScale` clamps it on read (deck-level fields bypass the slide gate — the same resolve-at-read defence as `resolveNotes`). 🟡 the **ink token** is now an honest readout of the canvas slide ink (`#0a0a0b`); ink *editing* is deferred — the canvas (light-bg/dark-text) vs export (dark-bg/white-text) colour-scheme divergence is a separate reconciliation.
- **Properties panel** 🟢 — `X/Y/W/H`, angle, opacity, and fill bind to the selected element live; for a **text** element, font **family**, **size**, **line spacing**, **align**, and **bold/italic/underline** are wired too (stored on the element, rendered by `ElementView`, committed via the same `onUpdateElement` path). Line spacing is a multiplier (`lineSpacingOf`, default 1.2, clamped positive) single-sourced across the canvas `line-height` and the export `lineSpacingMultiple` — so multi-line text matches (it previously diverged: canvas 1.2 vs the export's pptx default). 🟢 A **box shape** (rect / rounded rect / ellipse — `isStrokeableShape` in `lib/shapes.js`) also gets a **stroke** (outline colour + px width + a **dash style** — solid / dashed / dotted): rendered as a CSS `border` (inset via `box-sizing` so the footprint holds; `border-style` is the dash), gate-validated (`stroke` hex + non-negative `strokeWidth` + `strokeDash` enum), and exported as a pptx `line` (dash → `dashType`). The pen **`path`** reuses this Stroke section (its stroke is its only colour, no fill), with the dash rendered as an SVG `stroke-dasharray` on its polyline. `STROKE_DASHES`/`dashArray`/`dashType`/`borderStyle` are single-sourced in `lib/elements.js` (the gate shares the same `STROKE_DASHES` vocabulary). A malformed/persisted dash (a gate-bypassing write) degrades to **solid** on both surfaces — `borderStyle` clamps the CSS keyword (an invalid `border-style` would otherwise drop the whole `border` shorthand and vanish the outline) and `dashType` returns nothing — the same defence `isRenderableShadow`/`isRenderableGradient` give the shadow/gradient. 🟢 **Clip-path polygons** (triangle/star/hexagon/…) also stroke now: a CSS border is clipped away, so the canvas overlays an SVG `<polygon>` outline (`clipPoints` parses the shape's `clip-path` into the same 0..1 points; `vectorEffect="non-scaling-stroke"` keeps the width in slide-px) over the unchanged clip-path fill div — so fill/gradient/shadow stay CSS while the outline draws. `isStrokeableShape` now includes the polygons, so the inspector Stroke control, the canvas, and the export `line` all enable together. Only the **line** shape (itself a stroke) takes no outline. 🟢 **Any** element also takes a **drop shadow** (a `shadow` toggle seeding `{ color, blur, x, y }`): rendered as a CSS `filter: drop-shadow` (which follows the painted shape, clip-path included), gate-validated as a nested object, and exported as a pptx outer `shadow` (x/y→polar offset/angle, px→pt, opacity = a shared soft alpha × the element opacity clamped to 0–100, so canvas == export). Because the server write paths (PUT `/api/deck`, MCP `update_slide`) bypass the deck gate, both surfaces guard on `isRenderableShadow` — a malformed/persisted shadow (missing field, non-hex colour, non-finite offset) is skipped on the canvas *and* the export rather than rendering an invalid filter / a corrupt pptx. `SHADOW_OPACITY`/`dropShadowCss`/`isRenderableShadow`/`DEFAULT_SHADOW` are single-sourced in `lib/elements.js`. 🟢 **Any shape** (box, clip polygon, or line — `isFillableShape`) also takes a **gradient fill** (a `gradient` toggle seeding `{ from, to, angle }`): rendered as a CSS `linear-gradient` that overrides the solid `fill`, gate-validated (hex stops + finite angle). 🟡 pptxgen has no shape gradient, so the export approximates it with the **midpoint blend** (`mixHex`) of the two stops — a documented degradation (canvas shows the gradient, PPTX a solid). Both surfaces share `isRenderableGradient`, so a malformed gradient falls back to the solid `fill` on each. `DEFAULT_GRADIENT`/`linearGradientCss`/`isRenderableGradient` are single-sourced in `lib/elements.js`.
- **Data panel** 🟢 — in-app authoring for the data-driven layouts (`DataPanel`): a chart slide gets a categories×series grid (`ChartDataEditor`), a roadmap slide gets a lanes/milestones editor with state selects (`RoadmapLanesEditor`), and agenda/list/kpi/split slides get a per-item editor with add / delete / move-up-down reorder (`ListItemsEditor`, parameterised by the layout's collection + field schema — `items` for agenda/list, `kpis` for KPI, `stats` for split); other layouts show a hint. The chart/roadmap editors render the same normalized model the canvas/export draw (`chartData` / `roadmapModel` — a data-less starter materializes its fallback as editable rows) and commit full `{ chart }` / `{ lanes, months, todayIndex }` patches (the roadmap materializes its whole model so the demo TODAY marker survives the fork); the items editor commits `{ <collection> }` (plus a `remapCollectionFmt`-migrated `{ fmt }` on reorder/delete, so per-item formatting follows its item). 🟢 A **table** slide gets a 2D grid editor (`TableDataEditor`) — edit every header/cell, add/delete a column or row — committing `{ columns }`/`{ rows }` plus a `remapTableFmt`-migrated `{ fmt }` on delete (the 2D analogue: `columns.C` + `rows.R.C` keys follow their cell). ⚪ Row/column reordering within the table is a follow-up. All route through the Co-pilot's validated patch path (`onApplyAIPatch` → `sanitizeSlidePatch`), so the inspector and the AI share one schema gate.
- **Notes panel** 🟢 — a **Speaker notes** tab (`NotesPanel`) for every slide: a textarea that materializes `resolveNotes(slide)` — the authored `slide.notes`, else the bundled `SPEAKER_NOTES` starter — so what's shown is exactly what the PPTX export (`addNotes`) emits (the same "fallback as editable" pattern as the chart/roadmap editors; the presenter shows the same, with its own nudge fallback for a slide left empty). An edit commits `{ notes }` through the shared patch gate (`onApply` → `sanitizeSlidePatch`, which now validates `notes` as a **string** — matching `resolveNotes`, which ignores a non-string and would otherwise persist an inert value); clearing to `''` is an intentional "no notes" (`resolveNotes` honours an empty string rather than falling back). Complements the Co-pilot's "Generate speaker notes" with a manual authoring path.
- **Animate panel** 🟡 — 🟢 the **Transition** controls are wired: **TYPE** (a select over `TRANSITION_TYPES` — none/fade/slide/morph) and **DUR** (ms) read the live `slide.transition` (else a none/480ms default) and commit the whole `{ type, duration }` through the shared patch gate (`onApply` → `sanitizeSlidePatch` → `isValidTransition` in `deckUtils.js`, the same path the Data/Notes panels use — gating it to a known type + a positive duration with no extra keys, the strict shape of `isShadow`/`isGradient`; the panel commits both halves so a single-field edit can't drop the other). 🟢 the **Co-pilot** can author it too — `transition` is enumerated in the `editSlide` prompt (`llmClient.js`), so an instruction like "add a fade transition" emits a gated patch. 🟢 the **presenter plays** it on advance — `transitionAnim` (`lib/transitions.js`) maps the transition to a CSS entrance (fade/slide/morph keyframes in `main.css`; the stage is keyed on the slide index only when it animates, so the entrance replays on advance to an animated slide while plain slides reconcile in place; 'morph' is approximated as a soft blur dissolve — entrances avoid `transform: scale`, which would corrupt `ScaledSlide`'s getBoundingClientRect measure). 🟡 the **Builds** list is now a real editor — add / delete / retype per-slide entrance builds (type ∈ fadeIn/riseIn/zoomIn), committed as a gated `slide.builds` array (`BUILD_TYPES`/`isValidBuild` in `deckUtils.js`, the same strict shape as the transition gate; settable by the Co-pilot/MCP too). ⚪ timing (delay/duration) controls, per-element targeting, and presenter playback of builds are follow-ups (v1 stores `{ type }` only — timing fields land with their controls). ⚪ The PPTX export carries no transition or build animations (pptxgenjs 3.12 exposes no animation API).
...
- 🟢 **Slide transitions play on advance** — the entering slide animates with its `slide.transition` (set in the Animate panel) via a CSS entrance: `transitionAnim` (`lib/transitions.js`) maps `{ type, duration }` to a `present-fade`/`-slide`/`-morph` keyframe, and the stage is keyed on the slide index so the animation re-runs each advance. A `none`/absent transition is an instant cut. (Editor-only — the PPTX export carries no transition.)
...
- ~~Rotate the selection overlay (resize handles + rotate knob) to track a rotated element, which also needs rotate-aware resize math.~~ ✅ Done — 🟢 the selection frame + hit-box rotate with the element (`rotStyle`), and a rotated element resizes along its local axes keeping the opposite edge fixed in screen space (`resizeRotated` in `lib/elements.js`, via `rotateVec`). ✅ The line visual/grab/resize gap is resolved — a `line`'s height **is** its stroke thickness (model == visual == hit-box == export), an adjustable dimension floored at `MIN_LINE_THICKNESS` (2px, thinner than the normal `MIN_SIZE`) rather than capped at 8px. So a rotated rule's handles track its real visual at any thickness. ✅ A thin rule is still easy to select/grab — its **click + selection-outline box pads to a `HIT_MIN` minimum** (`hitBox` in `lib/elements.js`, centred on the element) while the visual stays at `el.h`; the resize handles deliberately stay on the real geometry (so a handle drag still tracks the true edge — no padded-frame disconnect).
- Selection set: shift-click multi-select ✅, marquee drag-rectangle ✅, shift-add-to-marquee ✅, align (horizontal + vertical) ✅, distribute ✅, and a rotate handle ✅ all shipped.
- 🟢 **Alignment guides** — dragging a single element snaps its edges/centre to other elements' edges/centres and the slide edges/centre (`alignSnap` in `lib/align.js`, within a ~6px tolerance), drawing a guide line per snapped axis; grid-snap still applies otherwise. (Multi-element drags stay grid-only.)
...
- ~~AI-authoring elements (the `elements` field isn't in the AI-patch whitelist).~~ ✅ Done — 🟢 `elements` is in the slide-patch gate (`deckUtils.js`): the whole array is validated against exactly what `ElementView` renders — a required string `id`, `type` single-sourced through `shapeDef` (shapes + text/image), finite `x/y/w/h`, a hex `fill` (required on every fill-bearing element — text/line/shape — so canvas == export; only `image` is exempt), a positive `fontSize`, an `align` of left/center/right, and a `data:image/` `src` (no remote fetch), and every key known + correctly typed (replace-not-merge: one bad entry rejects the field; an own-property lookup so a JSON-parsed `__proto__` key is rejected, not called). On apply (`Editor.applyAIPatch`) the order is **mint ids → validate → clamp**: `assignElementIds` (`lib/elements.js`) mints fresh unique ids on the raw elements (outside the reducer, StrictMode-safe), the gate validates strictly (no coercion launders a bad value), then each validated element is clamped to the slide like a manual create. The Co-pilot's `editSlide` prompt describes the full overlay vocabulary — text/shapes/lines (the shape list single-sourced from the `SHAPES` registry, so it can't drift) **plus the styling fields the gate accepts**: `stroke`/`strokeWidth`/`strokeDash` (dashed/dotted outlines), `gradient`, `shadow`, `opacity`, `rot`, and text `lineSpacing` — so it can author styled elements (a dashed callout, a gradient card, a drop-shadowed title), not just flat fills. **Existing image elements are preserved automatically** (`mergeOverlay` keeps the slide's images *at their stack/paint position* and drops any image in the AI reply) — the model can't round-trip a (large) data URL in its response, so images stay user-managed; the AI overlay replaces only the text/shape/line elements. (⚪ The Co-pilot can't create or reposition images.)
