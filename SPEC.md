...
| Tools | Select / Pen / Image, 10 Shapes | 🟢 a **shape tool draws** on the canvas — drag to size (or click for a default), creating an on-slide `slide.elements` shape (grid-snapped + bounds-clamped via `snapDrawnBox`/`clampElement`), then reverting to Select; **Text Box** & **Image** insert an element (Image via the file picker), and all element-creating actions exit draw mode. 🔴 **Pen** still sets the active tool only (freehand ⚪) |
