...
- ~~AI-authoring elements (the `elements` field isn't in the AI-patch whitelist).~~ ✅ Done — 🟢 `elements` is in the slide-patch gate (`deckUtils.js`): the whole array is validated against exactly what `ElementView` renders — `type` single-sourced through `shapeDef` (shapes + text/image), finite `x/y/w/h`, and every key known + correctly typed (replace-not-merge: one bad entry rejects the field). On apply, `normalizeAIElements` (`lib/elements.js`) mints fresh unique ids + clamps each to the slide (like a manual create), minted outside the reducer for StrictMode determinism. The Co-pilot's `editSlide` prompt describes the overlay schema so it can author text/shapes/lines. (⚪ It can't author `image` elements — data URLs — and a per-element edit replaces the whole overlay.)
