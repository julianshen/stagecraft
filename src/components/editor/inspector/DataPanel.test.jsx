...
describe('DataPanel', () => {
  it('routes chart/roadmap/agenda/list/kpi/split/table slides to their data editors', () => {
    ...
    render(<DataPanel slide={{ id: 'k', layout: 'kpi', kpis: [] }} onApply={vi.fn()} />);
    expect(screen.getByText('KPI items')).toBeInTheDocument();
    render(<DataPanel slide={{ id: 'sp', layout: 'split', stats: [] }} onApply={vi.fn()} />);
    expect(screen.getByText('Stat items')).toBeInTheDocument();
    render(<DataPanel slide={{ id: 'tb', layout: 'table', columns: [], rows: [] }} onApply={vi.fn()} />);
  ...
  it('shows a hint for slides without editable data, and when nothing is selected', () => {
    ...
    expect(screen.getByText(/chart, roadmap, agenda, list, KPI, split, and table slides/i)).toBeInTheDocument();
